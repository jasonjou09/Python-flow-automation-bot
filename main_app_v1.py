# -*- coding: utf-8 -*-
"""
Created on Fri Jul 10 19:47:07 2026
...
class TaxBillingApp:
    def __init__(self, root):
        self.root = root
        self.root.title("會計報稅請款整合系統 v6.0")
    ...
    def setup_billing_ui(self):
        ...
        for m in self.months:
            ...
            cols = ('status', 'name', 'bank_account', 'tax_amt', 'accounting_fee', 'special_fee', 'total_amt', 'note', 'id')
        ...
        # 將按鈕設為類別變數，並綁定新的切換函式 toggle_paid_status
    ...
    def check_and_initialize_billing(self, year, month_period):
        ...
        for c_id, base_fee in active_comps:
            ...
            if not self.db.cursor.fetchone():
                ...
                self.db.cursor.execute('''
                    INSERT INTO billing_records (company_internal_id, year, month_period, business_tax_amount, accounting_fee, note, is_paid)
                    VALUES (?, ?, ?, 0.0, ?, '', 0)
    ...
    def on_tree_select(self, event):
        ...
        self.selected_record_id = values[8] # 隱藏的紀錄 ID
        
        # 【修改點】在 SQL 中多撈取 r.is_paid 狀態
        self.db.cursor.execute('''
            SELECT c.company_code, c.full_name, r.business_tax_amount, r.accounting_fee, r.note, r.is_paid
            FROM billing_records r JOIN companies c ON r.company_internal_id = c.id WHERE r.id = ?
        ''', (self.selected_record_id,))
        res = self.db.cursor.fetchone()
        if not res: return
        code, name, tax, fee, note, is_paid = res
        ...
        # 【新增邏輯】根據付款狀態，動態改變按鈕文字、顏色與內部狀態標記
    ...
    def save_company_info(self):
            ...
            if getattr(self, 'selected_company_id', None):
                ...
                # 【新增修正】智慧連動更新：保護歷史紀錄的同時，允許連動修改當前未付款的帳單
    ...
    def refresh_treeview(self, month):
        ...
        # 請款明細左側一律以公司編號遞增排序
        self.db.cursor.execute('''
            SELECT c.company_code, r.is_paid, c.full_name, c.bank_account, r.business_tax_amount, r.accounting_fee, r.note, r.id
        ...
        for code, is_paid, name, bank, tax, fee, note, rid in self.db.cursor.fetchall():
            ...
            tree.insert('', tk.END, text=code, values=(status_icon, name, bank if bank else "", tax, fee, sp_sum, total_amt, note if note else "", rid))
